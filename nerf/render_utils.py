...
def get_rays(H, W, K, c2w):
    """
    Compute ray origins and directions for a batch of pixels.
    Args:
        H (int): Image height.
        W (int): Image width.
        K (array): Camera intrinsic matrix.
        c2w (array): Camera-to-world transformation matrix.
    Returns:
        rays_o (array): Ray origins.
        rays_d (array): Ray directions.
    """
    i, j = torch.meshgrid(
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    """
    raw2alpha = lambda raw, dists, act_fn=func.relu: 1.0 - torch.exp(
        -act_fn(raw) * dists
    )
